// Load dotenv only if it exists (not in Next.js server context)
try {
  require('dotenv').config();
} catch (e) {
  // Silently fail if dotenv not available (will use process.env from Next.js)
}
...
async function extractDesignSystem($, url) {
  ...
  try {
    // Use Puppeteer to extract colors from rendered page
    const puppeteer = require('puppeteer');
    
    console.log('      Launching browser for color extraction...');
    ...
    console.log('      Analyzing rendered elements...');
    
    // Extract colors from rendered DOM
    const colorData = await page.evaluate(() => {
      ...
      document.querySelectorAll('button, a[role="button"], input[type="submit"], [class*="btn"]').forEach(btn => {
        const bgColor = getColor(btn, 'backgroundColor');
        const textColor = getColor(btn, 'color');
        const borderColor = getColor(btn, 'borderColor');
        
        if (bgColor && bgColor !== 'rgba(0, 0, 0, 0)') {
          results.buttons.push({ color: bgColor, weight: 10 });
        }
        if (textColor && textColor !== 'rgba(0, 0, 0, 0)') {
          results.buttons.push({ color: textColor, weight: 8 });
        }
        if (borderColor && borderColor !== 'rgba(0, 0, 0, 0)') {
          results.buttons.push({ color: borderColor, weight: 7 });
        }
      });
      ...
      document.querySelectorAll('h1, h2, h3, h4, h5, h6').forEach(heading => {
        ...
        if (color && color !== 'rgba(0, 0, 0, 0)') {
          results.headings.push({ color, weight });
        }
      });
      ...
      document.querySelectorAll('nav a, header a').forEach(link => {
        ...
        if (color && color !== 'rgba(0, 0, 0, 0)') {
          results.links.push({ color, weight: 5 });
        }
      });
    ...
    console.log(`      Found ${colorData.buttons.length} button colors, ${colorData.headings.length} heading colors`);
    ...
    const isValidColor = (hex) => {
      ...
      if (r > 240 && g > 240 && b > 240) return false; // Too light
      if (r < 30 && g < 30 && b < 30) return false; // Too dark
      
      const maxDiff = Math.max(Math.abs(r - g), Math.abs(g - b), Math.abs(r - b));
      if (maxDiff < 15) return false; // Too gray
      
      return true;
    };
    
    // Process colors with weights
    const colorFrequency = {};
    
    [...colorData.buttons, ...colorData.headings, ...colorData.links].forEach(({ color, weight }) => {
    ...
    // Get top 3 colors
    colors = Object.entries(colorFrequency)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 3)
      .map(([color]) => color);
    
    console.log(`      Extracted ${colors.length} valid colors`);
  ...
  while (colors.length < 3) {
    const defaults = ['#3b82f6', '#1e40af', '#0ea5e9'];
    colors.push(defaults[colors.length]);
  }
...
module.exports = { extractBusinessInfo };
