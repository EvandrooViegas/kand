...
export function normalizeGroupGaps(group) {
  ...
  // Default gap depends on layout: vertical groups space on Y axis, horizontal on X axis
  const defaultGap = group.layout === 'vertical'
    ? { gapX: 0, gapY: 16 }
    : { gapX: 16, gapY: 0 }
  while (gaps.length < count) gaps.push({ ...defaultGap })
...
export function applyGroupLayoutToNodes(nodes, group) {
  ...
  for (let i = 1; i < ids.length; i++) {
    ...
    if (group.layout === 'vertical') {
      // For vertical layout: stack nodes top-to-bottom using gapY as the spacing.
      // Use the anchor (first node) x so the cross-axis position doesn't drift
      // as nodes grow — gapX here is a fixed offset from the anchor, not accumulated.
      positions.set(ids[i], {
        x: Math.round(first.x + gapX),
        y: Math.round(prevPos.y + prev.height + gapY),
      })
    } else {
      // For horizontal layout: stack nodes left-to-right using gapX as the spacing.
      // Use the anchor (first node) y so the cross-axis position doesn't drift
      // as nodes grow — gapY here is a fixed offset from the anchor, not accumulated.
      positions.set(ids[i], {
        x: Math.round(prevPos.x + prev.width + gapX),
        y: Math.round(first.y + gapY),
      })
    }
  }
