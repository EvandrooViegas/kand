...
export function applyGroupLayoutToNodes(nodes, group) {
  ...
  // For vertical groups: use the leftmost x position (considering offsets)
  // For horizontal groups: use the topmost y position (considering offsets)
  // 
  // IMPORTANT: When dynamic text grows in height, the main-axis positions shift,
  // but the cross-axis origin should be recalculated from FINAL positions to avoid
  // centering gaps when nodes have different heights.
  //
  // We calculate this BEFORE Pass 1 so it's stable, but it accounts for the actual
  // visual starting positions of all members.
