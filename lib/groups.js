...
export function applyGroupLayoutToNodes(nodes, group) {
  ...
  // The cross-axis origin is stored on the group so it never drifts across reflows.
  // For vertical layout: originX = the group's stable visible-left edge.
  // For horizontal layout: originY = the group's stable visible-top edge.
  // Fall back to first node's current position only if not yet stored.
  const crossOrigin = group.layout === 'vertical'
    ? (typeof group.originX === 'number' ? group.originX : first.x + firstDims.offsetX)
    : (typeof group.originY === 'number' ? group.originY : first.y + firstDims.offsetY)
