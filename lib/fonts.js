...
export async function loadFontsForCanvas(canvas) {
  ...
  for (const n of canvas.nodes || []) {
    ...
    // Inline font emphasis needs its font included in the exported PNG as well.
    for (const match of (n.text || '').matchAll(/(?:fontFamily|font-family)=([^|:%]+)(?=[|:])/g)) {
      const inlineFamily = match[1].trim()
      if (FONT_CONFIG[inlineFamily]) add(inlineFamily, weight, style)
    }
  }
