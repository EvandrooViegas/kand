  ...
  const childPromises = (canvas.nodes || []).map(async (node) => {
    ...
    if (node.type === 'text') {
      ...
      const style = {
        ...
        textAlign: align, lineHeight: 1.2, overflow: 'visible', whiteSpace: 'pre-wrap',
    ...
    if (node.type === 'image') {
      ...
      const imageSrc = hasValue ? String(dynVal) : node.src
      ...
      if (!buf) {
        return {
          type: 'div', props: {
            style: { position: 'absolute', left: node.x, top: node.y, width: node.width, height: node.height, background: '#e5e7eb', display: 'flex', borderRadius: radius, transform: node.rotation ? `rotate(${node.rotation}deg)` : undefined }
          }
        }
      }
      ...
      return {
        type: 'div', props: {
          style: { position: 'absolute', left: node.x, top: node.y, width: node.width, height: node.height, display: 'flex', overflow: 'hidden', borderRadius: radius, transform: node.rotation ? `rotate(${node.rotation}deg)` : undefined },
