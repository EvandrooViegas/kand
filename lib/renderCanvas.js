...
function calculateTextHeight(text, fontSize, width, lineHeight = 1.2) {
  const size = fontSize || 48
  const charWidth = size * 0.55 // Approximate character width
  const charsPerLine = Math.floor((width || 200) / charWidth)

  // Split by newlines and calculate wrapped lines
  const lines = text.split('\n').reduce((total, line) => {
    if (!line) return total + 1
    return total + Math.ceil(line.length / Math.max(1, charsPerLine))
  }, 0)
...
export async function renderCanvasToPng(canvas, data = {}) {
  ...
  // Track which nodes are in groups for special rendering
  ...
  if (canvas.groups && canvas.groups.length > 0) {
    ...
    // Expand grouped text nodes to fit their content
    nodes = nodes.map(node => {
      if (!groupedNodeIds.has(node.id) || node.type !== 'text') return node
      ...
      const calculatedHeight = calculateTextHeight(text, node.fontSize || 48, node.width, node.lineHeight || 1.2)
      return { ...node, height: calculatedHeight }
    })
  }

  if (canvas.groups && canvas.groups.length > 0) {
    console.log('Applying groups to nodes:', { groupCount: canvas.groups.length, nodeCount: nodes.length })
    for (const group of canvas.groups) {
      console.log(`Before applying group ${group.id}:`, nodes.filter(n => group.nodeIds.includes(n.id)).map(n => ({ id: n.id, x: n.x, y: n.y, width: n.width, height: n.height })))
      console.log(`Applying group ${group.id}:`, { layout: group.layout, nodeIds: group.nodeIds, gaps: group.gaps })
      nodes = applyGroupLayoutToNodes(nodes, group)
      console.log(`After applying group ${group.id}:`, nodes.filter(n => group.nodeIds.includes(n.id)).map(n => ({ id: n.id, x: n.x, y: n.y, width: n.width, height: n.height })))
    }
  }
