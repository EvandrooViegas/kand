...
import { normalizeGroupGaps } from './groups'
...
export async function renderCanvasToPng(canvas, data = {}) {
  ...
  const childPromises = nodes.map(async (node) => {
    ...
    if (node.type === 'image') {
      ...
      if (!buf) {
        const style = { position, width: node.width, marginTop, marginLeft, background: '#e5e7eb', display: 'flex', borderRadius: radius }
        if (left !== undefined) style.left = left
        if (top !== undefined) style.top = top
        if (height !== undefined) style.height = height
        if (node.rotation) style.transform = `rotate(${node.rotation}deg)`
      ...
      const style = { position, width: node.width, marginTop, marginLeft, display: 'flex', overflow: 'hidden', borderRadius: radius }
      if (left !== undefined) style.left = left
      if (top !== undefined) style.top = top
      if (height !== undefined) style.height = height
      if (node.rotation) style.transform = `rotate(${node.rotation}deg)`
