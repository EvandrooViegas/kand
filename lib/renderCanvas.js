...
function estimateTextHeight(text, node) {
  ...
  // Average character width varies by font/weight, ~0.55em is a reasonable estimate
  const charWidth = fontSize * 0.55
  const charsPerLine = Math.max(1, Math.floor(width / charWidth))
  const lines = text.split('\n').reduce((total, line) => {
    if (!line) return total + 1
    // Strip styled text tags to count visible chars
    const plain = line.replace(/<%[^%>]*%>/g, (m) => {
      const inner = m.match(/:([^%]*)%>$/)
      return inner ? inner[1] : ''
    })
    return total + Math.ceil(Math.max(1, plain.length) / charsPerLine)
  }, 0)
...
export async function renderCanvasToPng(canvas, data = {}) {
  ...
  if (canvas.groups && canvas.groups.length > 0) {
    ...
    nodes = nodes.map(node => {
      ...
      return estimatedH > node.height ? { ...node, height: estimatedH } : node
    })
  ...
  const childPromises = nodes.map(async (node) => {
    ...
    if (node.type === 'text') {
      ...
      const style = {
        position, left, top,
        width: node.width,
        // For grouped text nodes let Satori size height to content so dynamic
        // text that wraps more lines isn't clipped by a stale fixed height.
        // Non-grouped nodes keep their fixed height (overflow:hidden crops as designed).
        ...(isGrouped ? {} : { height: node.height }),
    ...
    if (node.type === 'image') {
      ...
      if (!buf) {
        const style = { position, left, top, width: node.width, height: node.height, background: '#e5e7eb', display: 'flex', borderRadius: wrapperRadius }
        if (node.rotation) style.transform = `rotate(${node.rotation}deg)`
        return {
          nodeId: node.id,
      ...
      // Apply physical crop via sharp.extract()
      ...
      // Apply SVG shape mask for polygon shapes
      if (useSvgMask) {
        try {
          const meta = await sharp(pngBuf).metadata()
      ...
      // Use effective (cropped) dimensions for the wrapper so it matches the visible area
      const effDims = getNodeEffectiveDimensions(node)
      const renderWidth = Math.round(effDims.width)
      const renderHeight = Math.round(effDims.height)
      const style = { position, left, top, width: renderWidth, height: renderHeight, display: 'flex', overflow: 'hidden', borderRadius: wrapperRadius }
      if (node.rotation) style.transform = `rotate(${node.rotation}deg)`
