...
export function brandDesignRequest(brand: any, existing: any[] = [], repair = false) {
 ...
 return {temperature:.8,max_tokens:3600,response_format:{type:'json_object'},messages:[
  ...
  {role:'user',content:JSON.stringify(context)+(repair?'\nThe previous attempt was incomplete or invalid. Return one complete design with cover, content and closing templates. Include headline and CTA roles in all three and body in content/closing, with text boxes separated by 12px.':'')}
 ]}
}
