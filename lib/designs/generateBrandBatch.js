 ...
 for(let index=0;index<count;index++) {
  ...
  for(let attempt=0;attempt<5;attempt++) {
   ...
   if(response.status===429&&attempt<4) {
    const providerWait=Number(response.headers.get('retry-after'))||61
    if(providerWait>120)throw new Error('AI quota reached. Retry in '+providerWait+' seconds. Existing designs are saved.')
