...
export const BLUEPRINT_BRIEF = `Author three original reusable visual systems, not selections from existing layouts. Return JSON {designs:[{name,tags,rationale,paletteId,blueprint:{version:1,identity,blockStyle,compositions,theme,spacing,highlight,imagery,complementary,templates}}]}.
compositions: choose 4 or more distinct composition rules in an ordered array from split,image-first,banner,editorial,poster,cards,statistic,closing. Order expresses this identity rhythm. Application selects by content purpose and length using code, with no AI call. blockStyle: markers|icons|cards|plain. Choose text-block presentation as part of the identity; do not always use markers. identity: concise art direction describing signature shapes, typography, depth, spacing and allowed layout variations. Templates are illustrative examples, not fixed layouts; posts are recomposed from this identity. Choose paletteId automatically for brand fit: brand|secondary|light|dark|bold. theme: studio|atmospheric|vibrant|technical. spacing: compact|balanced|generous.
highlight: none|underline|color|background|font|gradient_text|gradient_background. gradient_background means a gradient highlight box behind solid letters. Choose ONE style for the entire design, applied selectively to concise phrases, never every slide.
...
export function normalizeBlueprint(raw:any, issues:string[]=[]): any | null {
 ...
 return {version:1,blockStyle:pick(raw.blockStyle,['markers','icons','cards','plain'],raw.theme==='technical'?'icons':raw.theme==='vibrant'?'cards':raw.spacing==='generous'?'plain':'markers'),compositions:[...new Set((Array.isArray(raw.compositions)?raw.compositions:COMPOSITIONS.slice(0,5)).filter((c:any)=>COMPOSITIONS.includes(c)))],identity:String(raw.identity||'').slice(0,700),theme:pick(raw.theme,['studio','atmospheric','vibrant','technical'],'studio'),spacing:pick(raw.spacing,['compact','balanced','generous'],'compact'),highlight:pick(raw.highlight,['none','underline','color','background','font','gradient_text','gradient_background'],'none'),complementary:false,imagery:{style:pick(raw.imagery?.style,['photograph','drawing'],'photograph'),subject:pick(raw.imagery?.subject,['person','object','scene','none'],'person'),placement:pick(raw.imagery?.placement,['cutout','background','framed','none'],'cutout'),brief:String(raw.imagery?.brief||'').slice(0,1200)},templates}
}
