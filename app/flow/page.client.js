...
// ── Post Viewer Modal ──────────────────────────────────────────────────────
function PostViewerModal({ post, canvases, open, onClose }) {
  ...
  const allNodes = canvas ? [...(canvas.nodes || []), ...(canvas.pages || []).flatMap(p => p.nodes || [])].filter(n => n.dynamic_key) : []
      ...
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 py-4">
          <div className="lg:col-span-2 space-y-2">
            <div className="relative w-full bg-muted rounded-2xl overflow-hidden flex items-center justify-center" style={{ aspectRatio: '1080/1080' }}>
              {isCarousel && carouselPages && carouselPages.length > 0 ? (
                ...
                <div className="flex flex-col items-center justify-center gap-2 text-muted-foreground">
                  ...
                  <a href={url} target="_blank" rel="noreferrer" className="text-xs text-muted-foreground hover:text-foreground underline">Download ZIP</a>
                </div>
              ) : !isCarousel && url ? (
            ...
            {url && !isCarousel && (
              <div className="text-right">
                <a href={url} target="_blank" rel="noreferrer" className="text-[11px] text-muted-foreground hover:text-foreground underline flex items-center justify-end gap-1">
            ...
            <div className="rounded-xl border-2 border-foreground/10 p-3 space-y-2">
              ...
              <div className="flex items-center gap-2">
                {post.status === 'accepted' ? (
                  <><CheckCircle className="w-4 h-4 text-[#9AB800]" /><span className="text-sm font-bold text-[#9AB800]">Accepted</span></>
                ) : post.status === 'rejected' ? (
                  <><XCircle className="w-4 h-4 text-destructive" /><span className="text-sm font-bold text-destructive">Rejected</span></>
                ) : (
                  <><Clock className="w-4 h-4 text-amber-600" /><span className="text-sm font-bold text-amber-600">Pending Review</span></>
                )}
              </div>
              {post.scheduledAt && (
                    ...
                    return (
                      <div key={key} className="text-[9px] space-y-0.5">
                        <div className="flex items-center gap-1 font-bold text-muted-foreground">
                          {node.type === 'image' ? <ImageIcon className="w-3 h-3" /> : <Type className="w-3 h-3" />}
                          <code className="font-mono">{key}</code>
                        </div>
                        {node.type === 'image' && value ? (
                          <img src={value} alt="" className="h-16 rounded object-cover border border-foreground/10" onError={e => e.target.style.display = 'none'} />
                        ) : node.type === 'text' && value ? (
                          <p className="text-foreground/70 break-words line-clamp-3">{value}</p>
                        ) : null}
                      </div>
                    )
                  })}
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader><p className="font-bold flex items-center gap-2"><Pencil className="w-4 h-4" />Edit Post — {post.canvasName}</p></DialogHeader>
        <div className="space-y-4">
          ...
          <div className="space-y-3 max-h-96 overflow-y-auto">
            <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Content Fields</p>
            {allNodes.length === 0 ? (
              <p className="text-xs text-muted-foreground italic">No dynamic fields in this canvas.</p>
            ) : (
              allNodes.map(node => {
                const key = node.dynamic_key
                return (
                  <div key={key} className="rounded-lg border border-foreground/10 p-3 space-y-1.5">
                    <div className="flex items-center gap-2">
                    ...
                    {node.type === 'image' ? (
                      <div className="space-y-1.5">
                        {data[key] && <img src={data[key]} alt="" className="h-20 rounded object-cover border border-foreground/10" onError={e => { e.target.style.display = 'none' }} />}
                        <Input className="h-8 text-xs" placeholder="Image URL" value={data[key] || ''} onChange={e => setData(d => ({ ...d, [key]: e.target.value }))} />
                      </div>
                    ) : (
                      <Textarea rows={3} className="text-xs" placeholder="Enter text content..." value={data[key] || ''} onChange={e => setData(d => ({ ...d, [key]: e.target.value }))} />
                    )}
                  </div>
                )
              })
...
// ── Generation Progress ───────────────────────────────────────────────────
function GenerationProgress({ flowId }) {
  ...
  return (
    <div className="rounded-2xl border-2 border-foreground/10 bg-[#D4FF00]/5 p-5 flex items-center gap-4 animate-in fade-in duration-300">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p key={currentStep} className="text-sm font-semibold animate-in fade-in slide-in-from-left-1 duration-200 truncate">
          {steps[currentStep] || 'Processing'}…
        </p>
        <div className="mt-2 flex gap-1">
          {steps.map((_, i) => (
            <div key={i} className={`h-0.5 flex-1 rounded-full transition-colors ${i < currentStep ? 'bg-foreground' : i === currentStep ? 'bg-[#D4FF00]' : 'bg-foreground/15'}`} />
          ))}
        </div>
        {connectionError && <p className="text-[9px] text-amber-600 mt-2">Updates paused (showing client-side progress)</p>}
      </div>
    </div>
  )
}
...
function StepGenerate({ flow, canvases, onGenerate, onUpdatePost, onRerender, generating, brand, tone, language, activeFlow }) {
  ...
  const PostCard = ({ post }) => {
    ...
    return (
      <div className={`rounded-xl border-2 overflow-hidden transition-all flex flex-col ${
        ...
        <div className="bg-muted relative overflow-hidden shrink-0 flex items-center justify-center max-h-96 cursor-pointer group" style={{ minHeight: '300px' }} onClick={() => setViewPost(post)}>
          {isCarousel && carouselPages && carouselPages.length > 0 ? (
            <div className="w-full h-full flex flex-col items-center justify-center">
              <img 
                src={carouselPages[carouselIndex].url} 
              ...
              />
              {carouselPages.length > 1 && (
                <div className="absolute bottom-0 left-0 right-0 flex items-center justify-between p-2 bg-black/20" onClick={e => e.stopPropagation()}>
                  <button
                  ...
                  <span className="text-xs text-white font-bold">{carouselIndex + 1} / {carouselPages.length}</span>
                  <button
                    onClick={(e) => { e.stopPropagation(); setCarouselIndex(Math.min(carouselPages.length - 1, carouselIndex + 1)) }}
                    disabled={carouselIndex === carouselPages.length - 1}
                    className="text-white hover:text-white/70 disabled:opacity-30 transition"
                  >
            ...
            <img src={url} alt="" className="max-w-full max-h-full object-contain group-hover:opacity-90 transition" />
          ) : (
        ...
        <div className="flex flex-col flex-1 min-h-0">
          <div className="p-3 border-b border-foreground/10">
            ...
            {post.caption && <p className="text-[9px] text-foreground/70 leading-snug line-clamp-2">"{post.caption}"</p>}
          </div>
        </div>

        <div className="flex border-t border-foreground/10 text-[10px]">
          <button onClick={() => setViewPost(post)} className="flex-1 h-8 text-muted-foreground hover:bg-muted transition flex items-center justify-center gap-1 font-semibold"><Maximize2 className="w-3.5 h-3.5" />View</button>
          <button onClick={() => setEditPost(post)} className="flex-1 h-8 text-muted-foreground hover:bg-muted transition flex items-center justify-center gap-1 border-l border-foreground/10"><Pencil className="w-3 h-3" />Edit</button>
          {url && <a href={url} target="_blank" rel="noreferrer" className="flex-1"><button className="w-full h-8 text-muted-foreground hover:bg-muted transition flex items-center justify-center border-l border-foreground/10"><Download className="w-3 h-3" /></button></a>}
          {post.status !== 'accepted' && <button onClick={() => onUpdatePost(post.id, { status: 'accepted' })} className="flex-1 h-8 text-[#9AB800] hover:bg-[#9AB800]/10 transition flex items-center justify-center border-l border-foreground/10"><CheckCircle className="w-3.5 h-3.5" /></button>}
          <button onClick={() => { if (confirm('Delete this post?')) onUpdatePost(post.id, { status: 'deleted' }) }} className="flex-1 h-8 text-destructive hover:bg-destructive/10 transition flex items-center justify-center border-l border-foreground/10"><Trash2 className="w-3.5 h-3.5" /></button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
          ...
          {[['pending', pending], ['accepted', accepted]].map(([tab, list]) => (
            <TabsContent key={tab} value={tab} className="space-y-4">
              {list.length === 0 ? <p className="text-center py-10 text-muted-foreground text-sm">No {tab} posts.</p> : (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">{list.map(p => <PostCard key={p.id} post={p} />)}</div>
              )}
            </TabsContent>
          ))}
        </Tabs>
      ...
      {viewPost && (
        <PostViewerModal post={viewPost} canvases={canvases} open={!!viewPost} onClose={() => setViewPost(null)} />
      )}
...
export default function FlowPageClient({ initialFlows, initialCanvases, initialGalleries }) {
  ...
  useEffect(() => {
    if (!flowIdParam && flows.length > 0) return // Already loaded from props
  ...
  const saveFlow = async (extra = {}) => {
    ...
    const body = { ...activeFlow, brandContext: brand, selectedCanvases, galleryId, tone, language, contentIdeas, extractedContext, ...extra }
    ...
    <div className="min-h-screen bg-[#FAF7F2] dark:bg-[#0E0D0B] text-foreground flex flex-col">
      ...
      <div className="flex-1 max-w-6xl w-full mx-auto px-6 py-8">
        <div key={step} className="animate-in fade-in slide-in-from-right-2 duration-300">
          ...
          {step === 4 && <StepGenerate flow={activeFlow} canvases={canvases} onGenerate={generate} onUpdatePost={updatePost} onRerender={rerenderPost} generating={generating} brand={brand} tone={tone} language={language} activeFlow={activeFlow} />}
